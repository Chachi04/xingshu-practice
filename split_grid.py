...
How it works:
  1. Finds the outer red-bordered rectangle in the page (via color thresholding
     + contour detection) and reads its 4 corners, even if the photo was taken
     at a slight angle.
  ...
  3. Detects the internal grid line positions on the rectified image and fits
     ...
     by ink).
  4. Crops each cell (with a small inward padding so the red lines aren't
...
both faint lines and the dotted per-cell guides. For 3000行书常用.pdf that means:
...
def get_outer_corners(img):
    b, g, r = cv2.split(img.astype(int))
    red_mask = (((r - g) > 30) & ((r - b) > 30) & (r > 100)).astype(np.uint8) * 255
    ...
    raise RuntimeError("Could not find a 4-cornered outer border. Crop closer to "
                        "the grid before running, or supply corners manually.")
...
def detect_grid_lines(warped, axis, n_lines_hint=None, thresh_frac=0.15):
    b, g, r = cv2.split(warped.astype(int))
    red_mask = (((r - g) > 15) & ((r - b) > 15) & (r > 60)).astype(np.uint8) * 255
    profile = red_mask.sum(axis=axis) / 255
    centers = find_line_centers(profile, profile.max() * thresh_frac)
    if len(centers) < 2:
        raise RuntimeError(f"found {len(centers)} grid line(s) along axis {axis}; "
...
def _pdf_page_count(path):
    ...
    if shutil.which("pdftoppm"):
        raise SystemExit("Need pdfinfo (poppler) to count PDF pages, or install PyMuPDF "
                         "(pip install pymupdf).")
    raise SystemExit("No PDF backend found. Install PyMuPDF (pip install pymupdf) or "
...
def render_pdf_pages(path, pages, dpi):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for n in pages:
            ...
            subprocess.run(["pdftoppm", "-r", str(dpi), "-f", str(n), "-l", str(n),
                            "-png", path, prefix], check=True)
...
def process_page(img, args, thresh, prefix):
    corners = get_outer_corners(img)
    warped, w, h = warp(img, corners)

    col_lines = detect_grid_lines(warped, axis=0, n_lines_hint=(args.cols + 1) if args.cols else None,
                                  thresh_frac=thresh)
    row_lines = detect_grid_lines(warped, axis=1, n_lines_hint=(args.rows + 1) if args.rows else None,
                                  thresh_frac=thresh)
    ...
    for r in range(n_rows):
        ...
        for c in range(n_cols):
            ...
            cv2.imwrite(os.path.join(args.output_dir, f"{prefix}row{r+1:02d}_col{c+1:02d}.png"), cell)
            count += 1
    return count
...
def main():
    ...
    ap.add_argument("--cols", type=int, default=None, help="number of character columns")
    ap.add_argument("--rows", type=int, default=None, help="number of character rows")
    ap.add_argument("--pad", type=int, default=6, help="inward padding in px to skip the grid line")
    ap.add_argument("--pages", default=None,
                    help="PDF only: page or range, e.g. 3, 3-7, 1,4,9-11 (default: all)")
    ap.add_argument("--dpi", type=int, default=200, help="PDF only: render resolution (default 200)")
    ap.add_argument("--line-thresh", type=float, default=None, dest="line_thresh",
                    help="grid-line detection threshold as a fraction of the max red profile "
    ...
    thresh = args.line_thresh if args.line_thresh is not None else (0.3 if is_pdf else 0.15)
    ...
    pages = parse_pages(args.pages, n_pages) if args.pages else list(range(1, n_pages + 1))
    ...
    for n, img in render_pdf_pages(args.input, pages, args.dpi):
        ...
        try:
            ...
            skipped.append(n)
    print(f"Saved {total} cell images from {len(pages) - len(skipped)} page(s) to {args.output_dir}/")
    if skipped:
